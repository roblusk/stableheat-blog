...
- The patio
- The waterfront dock
- The stair runs connecting different elevations
- The element layout under each
- The transformer and control infrastructure
- The FLIR-documented commissioning

Most snowmelt projects we work on cover one or two of these. Chadfield covered all of them. The result is a residence where the entire outdoor circulation surface is engineered to clear itself.

## Why the dock is the interesting one
...
This kind of post-install thermal verification is something we now recommend on every multi-zone snowmelt project. It catches commissioning issues at the moment they're cheapest to fix, and it gives the homeowner documented proof that the system was operating correctly at handover.

## Lessons for similar properties

For lakefront or large multi-elevation residential properties considering snowmelt, the Chadfield approach offers a useful template:

1. **Treat each surface as its own zone.** Driveways, patios, docks, and stairs all have different thermal behaviors. A unified control system with per-zone heating elements respects those differences.
2. **Plan controls during slab pour.** Embedding sensor wires and feed conduits during construction is much cheaper than retrofitting later.
3. **Document with thermal imaging at commissioning.** Cheap insurance, lasting value.

The Chadfield property doesn't close down for winter. The walks stay clear, the dock stays usable, and the homeowner uses the property the same way in February as in July.
